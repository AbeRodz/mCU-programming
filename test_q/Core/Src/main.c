...
/**
 * @brief Writes/updates the duration on a defined delay.
 * Function that writes a duration value on a delay.
 * @param delay Pointer to a defined delay struct.
 * @param duration new duration value in milliseconds.
 * @note if duration is 0 or a negative number, it defaults to DEFAULT_DELAY.
 */
void delayWrite(delay_t *delay,tick_t duration){
	if (duration <= 0){
